...
def _run_xfoil(
    ...
    Args:
        airfoil: Instance de profil aérodynamique AeroSandbox.
        re_val: Nombre de Reynolds de l'écoulement.
        alphas: Tableau numpy des angles d'attaque à évaluer.
        max_iter: Nombre maximal d'itérations XFoil par point.
        timeout: Temps limite alloué à l'analyse complète de la séquence.

    Returns:
        Un dictionnaire associant l'angle d'attaque arrondi à son triplet
...
def _is_physical(cl: float, cd: float, cm: float) -> bool:
    ...
    Returns:
        True si toutes les valeurs sont réalistes, False sinon.
    """
    return (
...
def _sanitize_map(
    ...
    Args:
        alpha_map: Dictionnaire brut issu de XFoil `{alpha: (CL, CD, CM)}`.

    Returns:
        Dictionnaire nettoyé ne contenant que les points jugés physiques.
    """
...
def _extrapolate(
    ...
    Args:
        alphas: Liste exhaustive de tous les angles d'attaque cibles.
        alpha_map: Dictionnaire des points valides convergés.

    Returns:
        Un dictionnaire complet couvrant l'ensemble de la plage d'angles d'attaque demandée.
    """
    if not alpha_map:
...
def compute_profile_re(args: Tuple[str, float, np.ndarray, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Args:
        args: Un tuple d'arguments contenant :
            - profil_name (str): Identifiant du profil.
            - re_val (float): Nombre de Reynolds à évaluer.
            - alphas (np.ndarray): Liste ordonnée des angles d'attaque cibles.
            - geom_row (Dict[str, Any]): Dictionnaire des métadonnées géométriques.

    Returns:
        Un dictionnaire résumant le succès de l'opération, le taux de convergence
        et la liste complète des dictionnaires de lignes générées pour le DataFrame.
    """
...
class XFoilDatasetBuilder:
    ...
    Attributes:
        input_csv (str): Emplacement du fichier de données géométriques d'entrée.
        output_csv (str): Emplacement du fichier CSV cible enrichi.
        max_cores (int): Nombre maximal de cœurs CPU autorisés pour la parallélisation.
    """
    ...
    def __init__(self, input_csv: str, output_csv: str, max_cores: int = 8) -> None:
        ...
        Args:
            input_csv: Chemin vers le CSV géométrique d'entrée.
            output_csv: Chemin vers le CSV enrichi de sortie.
            max_cores: Plafond du nombre de cœurs processeurs utilisables.
        """
    ...
    def _flush_rows(
        ...
        Args:
            rows: Liste des dictionnaires de données générés pour un profil à un Reynolds donné.
            output_csv: Chemin cible du fichier CSV d'écriture.
            first_write_flag: Liste mutable d'un élément booléen contrôlant l'ajout du header.
            global_row_counter: Liste mutable d'un entier traquant le nombre total de lignes écrites.
            t0: Horodatage initial du lancement du script.
            n_total_for_eta: Volume total estimé de lignes à écrire pour le calcul de l'ETA.
            profil: Identifiant du profil aérodynamique courant.
            re_val: Nombre de Reynolds courant.
        """
        for row in rows:
