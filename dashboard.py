...
def _tracer_polaire_prediction(df: pd.DataFrame, disposition: str, df_flu: pd.DataFrame = None) -> None:
    """Trace les graphiques de polaires à partir du ML et superpose Fluent si disponible."""
    couleur = COLORS["accent"]

    spec_courbes = [
        ("CL",      "Coefficient de portance",  "c<sub>l</sub>", "CL"),
        ("CD",      "Coefficient de traînée",   "c<sub>d</sub>", "CD"),
        ("CM",      "Coefficient de moment",    "c<sub>m</sub>", "CM"),
        ("finesse", "Coefficient de finesse",   "c<sub>l</sub> / c<sub>d</sub>", "LD"),
    ]

    figs = []
    for col_data, titre_long, titre_court, flu_col in spec_courbes:
        fig = go.Figure()
        
        # Courbe du ML (intacte)
        fig.add_trace(go.Scatter(
        ...
        # Ajout des points Fluent
        if df_flu is not None and flu_col in df_flu.columns:
            fig.add_trace(go.Scatter(
        ...
        fig.update_layout(
            title=dict(text=f"<b>{titre_long}</b>", x=0.5, y=0.95,
                       xanchor="center", yanchor="top", xref="paper", font=dict(size=16, color=COLORS["primary"])),
            xaxis=dict(title=dict(text="<b>Angle d'attaque α (°)</b>", font=dict(size=12)), showgrid=True, gridwidth=1, gridcolor="lightgray", showline=True, linewidth=2, linecolor="black", mirror=True, ticks="outside", tickwidth=2, ticklen=8),
            yaxis=dict(title=dict(text=f"<b>{titre_court}</b>", font=dict(size=12)), showgrid=True, gridwidth=1, gridcolor="lightgray", showline=True, linewidth=2, linecolor="black", mirror=True, ticks="outside", tickwidth=2, ticklen=8),
            height=430, margin=dict(t=80, b=50, l=65, r=30),
            legend=dict(bgcolor="rgba(255,255,255,0.9)", bordercolor="black", borderwidth=1, font=dict(size=11), yanchor="top", y=0.98, xanchor="left", x=0.02),
            plot_bgcolor="white", paper_bgcolor=COLORS["background"], hovermode="x unified",
        )
        figs.append(fig)
...
def page_prediction_ml(disposition: str, fluent: dict) -> None:
    """Page 5 : génération des polaires ML pour un profil quelconque."""
    ...
    presets = {
        "— Personnalisé —"  : None,
        "SG_6043 (Fluent)" : dict(t=0.100, camber=0.051, x_t=0.321, x_c=0.533, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.3115, area=0.05, default_re=100_000),
        "NASA_SC_0714 (Fluent)" : dict(t=0.140, camber=0.025, x_t=0.370, x_c=0.810, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.3784, area=0.05, default_re=500_000),
        "MH_45 (Fluent)" : dict(t=0.0985, camber=0.017, x_t=0.269, x_c=0.366, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.3662, area=0.05, default_re=100_000),
        "EPPLER_E817 (Fluent)" : dict(t=0.110, camber=0.029, x_t=0.329, x_c=0.667, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.3343, area=0.05, default_re=500_000),
        "Wortmann_FX60_126 (Fluent)" : dict(t=0.101, camber=0.036, x_t=0.308, x_c=0.597, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.3279, area=0.05, default_re=1_000_000),
        "GOE 101 (Fluent)" : dict(t=0.069, camber=0.040, x_t=0.300, x_c=0.300, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.2000, area=0.05, default_re=500_000),
        "Supermarine 371-II (Fluent)" : dict(t=0.084, camber=0.009, x_t=0.500, x_c=0.450, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.0840, area=0.05, default_re=100_000),
        "NACA 0106 (Fluent)" : dict(t=0.060, camber=0.000, x_t=0.300, x_c=0.000, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.2000, area=0.05, default_re=50_000),
        "NACA 0012 (Fluent)" : dict(t=0.120, camber=0.000, x_t=0.300, x_c=0.300, LE_radius=0.01, TE_angle=0.0, t_over_xt=0.4000, area=0.05, default_re=400_000),
        "NACA 2412 (Classique)": dict(t=0.12, camber=0.02, x_t=0.30, x_c=0.40, LE_radius=0.0158, TE_angle=12.0, t_over_xt=0.400, area=0.0800, default_re=1_000_000),
        "NACA 4412 (Classique)" : dict(t=0.12, camber=0.04, x_t=0.30, x_c=0.40, LE_radius=0.0158, TE_angle=12.0, t_over_xt=0.400, area=0.0832, default_re=1_000_000),
    }
    ...
    for idx, (feat, (label, f_min, f_max, f_def, f_step, aide)) in enumerate(items):
        ...
        with col:
            geo[feat] = st.slider(label, min_value=f_min, max_value=f_max, value=float(valeur_init), step=f_step, format=f"%.{decimales}f", help=aide, key=f"slider_{feat}")
    ...
    # Mise à jour des valeurs RE dans le slider
    ...
    # --- Liaison indépendante avec Fluent ---
    fluent_keys_mapping = {
        "SG_6043 (Fluent)": "SG_6043",
        "NASA_SC_0714 (Fluent)": "NASA_SC_0714",
        "MH_45 (Fluent)": "MH_45",
        "EPPLER_E817 (Fluent)": "EPPLER_E817",
        "Wortmann_FX60_126 (Fluent)": "Wortmann_FX60_126",
        "GOE 101 (Fluent)": "goe101",
        "Supermarine 371-II (Fluent)": "supermarine371ii",
        "NACA 0106 (Fluent)": "naca0106",
        "NACA 0012 (Fluent)": "naca0012"
    }

    df_flu_to_plot = None
    if preset_choisi in fluent_keys_mapping:
        fluent_key = fluent_keys_mapping[preset_choisi]
        if (fluent_key, float(re_val)) in fluent:
            df_flu_to_plot = fluent[(fluent_key, float(re_val))]
            st.info(f"✨ Données Ansys Fluent superposées au graphique.")
    ...
    _tracer_polaire_prediction(df_pred, disposition, df_flu_to_plot)

    with st.expander("Voir les données tabulaires"):
        df_affiche = df_pred.copy()
        df_affiche.columns = ["α (°)", "CL", "CD", "CM", "CL/CD"]
        st.dataframe(df_affiche.style.format({"α (°)": "{:.2f}", "CL": "{:.5f}", "CD": "{:.6f}", "CM": "{:.5f}", "CL/CD": "{:.2f}"}), use_container_width=True, hide_index=True)
