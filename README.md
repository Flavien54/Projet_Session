...
# 3. Installer TOUTES les dépendances (data, ML, doc, tests…)
pip install -r requirements.txt
```

> `requirements.txt` regroupe l'ensemble des dépendances Python du projet
> (y compris `tensorflow`, `aerosandbox` et `sphinx`). Seul **XFoil** s'installe
> à part car ce n'est pas un paquet Python (voir Prérequis).
...
```bash
# Sphinx est déjà installé via requirements.txt

# 1. (déjà fait dans ce dépôt) créer le squelette du projet
python -m sphinx.cmd.quickstart docs --sep

# 2. Générer/régénérer les fichiers .rst à partir des docstrings
#    (-f pour écraser/rafraîchir les .rst existants, ex. nouveau module)
python -m sphinx.ext.apidoc -f -o docs/source . docs tests

# 3. Construire le site HTML
cd docs
python -m sphinx -b html source build/html

# 4. Ouvrir le résultat en local (double-clic sur le fichier)
...
## Dépendances selon l'étape

Toutes les dépendances **Python** sont déjà installées par `requirements.txt`.
Ce tableau indique seulement à quelle étape chacune sert réellement :

| Dépendance | Quand nécessaire | Fournie par |
|---|---|---|
| `tensorflow` | Étapes 3 et 4 (entraînement / inférence) | `requirements.txt` |
| `aerosandbox` | Étapes 1 et 2 (génération dataset + XFoil) | `requirements.txt` |
| XFoil (exécutable) | Étape 2 uniquement | Téléchargement manuel (MIT) — **pas un paquet Python** |

> Le dashboard (étape 6) et l'audit (étape 5) fonctionnent sans XFoil grâce aux
> données pré-calculées fournies.
